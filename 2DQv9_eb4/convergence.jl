...
function eigen_qubit_conv(
    ...
    eigen_qubit(21.70551, num_eigens, 0, target_freq;
...
function eigen_resonator_conv(refine::Real, order::Int, test_id::Int=0, num_eigens::Int=1, target_freq::Real=5.0)
    output_path = joinpath(
        OUTPUT_DIR,
        "resonator",
        "convergence_test_#$(test_id)",
    ...
    eigen_resonator(num_eigens, 0, target_freq;
        refine=refine,
        order=order,
        output_path=output_path
    )
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for order in range(3, 6)
        for refine in range(1, 8 - order)
            # eigen_qubit_conv(refine / 2 - 0.5, order, 2, 3.0, true)
